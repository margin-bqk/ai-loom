...
from src.loom.memory.world_memory import (
    WorldMemory, MemoryEntity, MemoryEntityType, 
...
class TestWorldMemory:
    ...
    @pytest.fixture
    def sample_relation(self, sample_entity):
        """创建示例关系"""
    ...
    async def test_add_and_get_relations(self, sample_entity, sample_relation):
        ...
        assert len(related) == 0  # 目标实体不存在，所以没有相关实体
...
class TestMemorySummarizer:
    ...
    def sample_entities(self):
        ...
        for i in range(8):
            entity = MemoryEntity(
                ...
                session_id="test-session",
                type=MemoryEntityType.CHARACTER if i % 2 == 0 else MemoryEntityType.EVENT,
                content={
                    "name": f"实体{i}",
    ...
    async def test_select_entities_for_summarization(self, sample_entities):
        ...
        # 选择的实体应该比保留天数更旧
        cutoff_time = datetime.now() - timedelta(days=summarizer.config.preserve_recent_days)
    ...
    async def test_cache_functionality(self, sample_entities):
        """测试缓存功能"""
        config = SummaryConfig(enable_cache=True, cache_ttl_hours=1)
