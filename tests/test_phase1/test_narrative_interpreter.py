...
class TestNarrativeInterpreterInterface:
    ...
    @pytest.mark.asyncio
    async def test_create_session_interface(self, mock_interpreter, sample_session_config):
        """测试创建会话接口"""
        # 设置模拟返回值
    ...
    async def test_load_session_interface(self, mock_interpreter):
        ...
        # 验证调用
        mock_interpreter.load_session.assert_called_once_with(session_id, force_reload=True)
        assert result == mock_session
    
    @pytest.mark.asyncio
    async def test_interpret_narrative_interface(self, mock_interpreter, sample_narrative_context):
        """测试解释叙事接口"""
        # 设置模拟返回值
        ...
        # 调用接口
        session_id = "test-session-123"
        result = await mock_interpreter.interpret_narrative(session_id, sample_narrative_context)
        
        # 验证调用
        mock_interpreter.interpret_narrative.assert_called_once_with(session_id, sample_narrative_context)
    ...
    async def test_check_consistency_interface(self, mock_interpreter):
        ...
        success, issues = await mock_interpreter.check_consistency(session_id, new_content)
        
        # 验证调用
        mock_interpreter.check_consistency.assert_called_once_with(session_id, new_content)
...
class TestNarrativeArchivePersistenceInterface:
    ...
    def sample_narrative_archive(self):
        ...
        return NarrativeArchive(
            ...
            key_characters=[
                {"name": "亚瑟", "role": "主角", "development": "成长中"}
            ],
            plot_arcs=[
                {"name": "寻找圣杯", "status": "进行中", "progress": 0.3}
            ],
            created_at=datetime.now(),
            updated_at=datetime.now(),
            version=1,
    ...
    @pytest.mark.asyncio
    async def test_save_narrative_archive_interface(self, mock_archive_persistence, sample_narrative_archive):
        """测试保存叙事档案接口"""
        # 设置模拟返回值
        mock_archive_persistence.save_narrative_archive.return_value = True
        
        # 调用接口
        result = await mock_archive_persistence.save_narrative_archive(sample_narrative_archive)
        
        # 验证调用
        mock_archive_persistence.save_narrative_archive.assert_called_once_with(sample_narrative_archive)
        assert result is True
    
    @pytest.mark.asyncio
    async def test_load_narrative_archive_interface(self, mock_archive_persistence, sample_narrative_archive):
        """测试加载叙事档案接口"""
        # 设置模拟返回值
        mock_archive_persistence.load_narrative_archive.return_value = sample_narrative_archive
        ...
        # 验证调用
        mock_archive_persistence.load_narrative_archive.assert_called_once_with(archive_id)
        assert result == sample_narrative_archive
        assert result.id == archive_id
    
    @pytest.mark.asyncio
    async def test_list_narrative_archives_interface(self, mock_archive_persistence, sample_narrative_archive):
        """测试列出叙事档案接口"""
        # 设置模拟返回值
        ...
        # 验证调用
        mock_archive_persistence.list_narrative_archives.assert_called_once_with(session_id)
        assert len(result) == 1
    ...
    async def test_export_to_markdown_interface(self, mock_archive_persistence):
        ...
        result = await mock_archive_persistence.export_to_markdown(archive_id, output_path)
        
        # 验证调用
        mock_archive_persistence.export_to_markdown.assert_called_once_with(archive_id, output_path)
        assert result is True
    ...
    async def test_create_archive_version_interface(self, mock_archive_persistence):
        ...
        result = await mock_archive_persistence.create_archive_version(archive_id, description)
        
        # 验证调用
        mock_archive_persistence.create_archive_version.assert_called_once_with(archive_id, description)
...
class TestInterfaceIntegration:
    ...
    async def test_interpreter_scheduler_integration(self):
        ...
        # 验证调用
        mock_interpreter.interpret_narrative.assert_called_once_with(session_id, context)
        mock_scheduler.schedule_narrative_event.assert_called_once_with(
    ...
    async def test_archive_persistence_integration(self):
        ...
        # 导出为Markdown
        export_result = await mock_persistence.export_to_markdown(archive.id, "./exports/archive.md")
        assert export_result is True
        
        # 验证调用
        mock_persistence.save_narrative_archive.assert_called_once_with(archive)
        mock_persistence.export_to_markdown.assert_called_once_with(archive.id, "./exports/archive.md")
