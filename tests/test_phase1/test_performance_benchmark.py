...
class TestPerformanceBenchmarks:
    ...
    async def test_narrative_interpretation_performance(self):
        ...
        for i in range(100):
            interpretation = NarrativeInterpretation(
                ...
                narrative_arcs=[
                    {"name": f"弧线{i}", "progress": i / 100.0} for _ in range(2)
                ],
            )
    ...
    def test_memory_usage_estimation(self):
        ...
        for i in range(100):
            config = SessionConfig(
                name=f"会话{i}", canon_path=f"./canon{i}.md", llm_provider="openai"
            )
    ...
    async def test_concurrent_interface_calls(self):
        ...
        # 并发执行
        tasks = [
            call_interface(iface, i * 0.0005) for i, iface in enumerate(mock_interfaces)
        ]
...
class TestComparativeBenchmarks:
    ...
    def test_before_after_refactoring_comparison(self):
        ...
        improvements = {
            "interface_call_time": (
                (
                    before_refactor["interface_call_time"]
                    - after_refactor["interface_call_time"]
                )
                / before_refactor["interface_call_time"]
            )
            * 100,
            "memory_per_object": (
                (
                    before_refactor["memory_per_object"]
                    - after_refactor["memory_per_object"]
                )
                / before_refactor["memory_per_object"]
            )
            * 100,
            "concurrent_throughput": (
                (
                    after_refactor["concurrent_throughput"]
                    - before_refactor["concurrent_throughput"]
                )
                / before_refactor["concurrent_throughput"]
            )
            * 100,
        ...
        print(f"\n重构前后性能对比（模拟）:")
        print(
            f"  接口调用时间: {before_refactor['interface_call_time']:.1f}ms -> {after_refactor['interface_call_time']:.1f}ms ({improvements['interface_call_time']:.1f}% 改进)"
        )
        print(
            f"  对象内存使用: {before_refactor['memory_per_object']}B -> {after_refactor['memory_per_object']}B ({improvements['memory_per_object']:.1f}% 改进)"
        )
        print(
            f"  并发吞吐量: {before_refactor['concurrent_throughput']}/s -> {after_refactor['concurrent_throughput']}/s ({improvements['concurrent_throughput']:.1f}% 改进)"
        )
