...
class TestWorldEditor:
    ...
    async def test_modify_rule(self):
        ...
        try:
            ...
            result = await editor.modify_rule(
                rule_name, "章节1", "新内容", "测试修改"
            )
    ...
    async def test_integrate_with_world_memory(self):
        ...
        mock_world_memory.get_memory_stats = AsyncMock(return_value={"entities": 5, "relations": 3})
        mock_world_memory.export_memory = AsyncMock(return_value={"entities": [], "relations": []})
    ...
    async def test_execute_comprehensive_edit_rule(self):
        ...
        with patch('builtins.open', create=True) as mock_open:
            ...
            result = await editor.execute_comprehensive_edit(edit_text, {"session_id": "test"})
    ...
    async def test_execute_comprehensive_edit_world(self):
        ...
        result = await self.editor.execute_comprehensive_edit(edit_text, {"session_id": "test"})
        
        # 由于没有WorldMemory，应该失败
        assert result.success == False
        assert "无法解析编辑命令" in result.narrative_impact or "世界记忆未初始化" in result.narrative_impact
