...
class TestBYOKMultiProviderIntegration:
    ...
    def setup_method(self):
        ...
        test_config = {
            ...
            "session_defaults": {"default_llm_provider": "openai_test"},
        }
    ...
    async def test_key_manager_integration(self):
        ...
        # 测试密钥掩码 - 需要找到正确的key_id
        # _keys字典使用key_id作为键，而不是provider名称
        key_info = None
        for k_id, k_info in key_manager._keys.items():
            if k_info.provider == "openai":
                key_info = k_info
                break

        assert key_info is not None, "Should have found openai key"
