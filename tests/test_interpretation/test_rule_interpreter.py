...
"""
    ...
    def test_constraint_extraction(self):
        ...
        # 检查禁止性规则
        prohibitions = [c for c in permission_constraints 
                       if c.metadata.get("is_prohibition", False)]
...
"""
        ...
        conflict_canon = MarkdownCanon(
            path=Path("test_conflict.md"),
            raw_content=conflict_content
        )
        ...
        permission_conflicts = [c for c in conflicts if c["type"] == "permission_conflict"]
    ...
    def test_parse_specific_sections(self):
        ...
        # 检查禁止性规则
        prohibitions = [p for p in permissions if p.metadata.get("is_prohibition", False)]
        assert len(prohibitions) >= 1
