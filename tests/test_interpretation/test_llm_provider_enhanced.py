...
class TestLLMProviderEnhancements:
    ...
    def test_llm_request_hash(self):
        """测试LLMRequest哈希生成"""
        request1 = LLMRequest(
            prompt="Hello world", model="gpt-3.5-turbo", temperature=0.7, max_tokens=100
        )
        ...
        request3 = LLMRequest(
            prompt="Hello world", model="gpt-4", temperature=0.7, max_tokens=100
        )
...
class TestKeyManager:
    ...
    def test_key_info_masking(self):
        """测试密钥掩码"""
        key_info = APIKeyInfo(
            provider="openai", key_id="test123", key_value="sk-test1234567890abcdef"
        )
        ...
        # 短密钥测试
        key_info_short = APIKeyInfo(provider="test", key_id="test", key_value="short")
...
class TestAsyncFunctionality:
    ...
    async def test_provider_manager_with_fallback(self):
        ...
        mock_provider2.generate.return_value = LLMResponse(
            content="Success from provider 2", model="test-model", usage={}, metadata={}
        )
    ...
    async def test_retry_with_backoff(self):
        ...
        # 测试重试成功
        result = await retry_with_backoff(failing_function, provider="test_provider")
...
class TestIntegration:
    ...
    def test_provider_factory_creation(self):
        ...
        # 测试Local Provider创建
        local_config = {"type": "local", "name": "test_local", "model": "llama2"}
