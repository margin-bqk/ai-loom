...
主角是一名年轻的法师学徒，正在探索古老的遗迹。""",
        ...
        # 测试截断功能
        truncated_context = prompt_assembler.truncate_to_fit_tokens(context, max_tokens=500)
        truncated_result = prompt_assembler.assemble(truncated_context)
        
        assert truncated_result.token_estimate <= 500 or truncated_result.token_estimate < result.token_estimate
    ...
    async def test_error_handling_integration(self):
        ...
        # 模拟会超时的处理逻辑
        original_execute = turn_scheduler._execute_turn
        turn_scheduler._execute_turn = AsyncMock(side_effect=asyncio.TimeoutError("模拟超时"))
