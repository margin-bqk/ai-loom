...
class TestSessionManager:
    ...
    def session_manager(self):
        ...
        manager = SessionManager(
            persistence_engine=persistence_mock,
    ...
    @pytest.mark.asyncio
    async def test_load_session_from_memory(self, session_manager, sample_session_config):
        """测试从内存加载会话"""
        # 先创建会话
        session_manager.persistence.save_session = AsyncMock(return_value=True)
        session = await session_manager.create_session(sample_session_config)
    ...
    async def test_load_session_from_persistence(self, session_manager):
        ...
        session_manager.persistence.load_session = AsyncMock(return_value=mock_session_data)
    ...
    async def test_list_sessions(self, session_manager, sample_session_config):
        ...
        # 归档一个会话
        await session_manager.update_session_status(sessions[0].id, SessionStatus.ARCHIVED)
    ...
    async def test_update_session_status(self, session_manager, sample_session_config):
        ...
        # 更新状态
        success = await session_manager.update_session_status(session.id, SessionStatus.PAUSED)
    ...
    @pytest.mark.asyncio
    async def test_cleanup_inactive_sessions(self, session_manager, sample_session_config):
        """测试清理不活跃会话"""
        # 创建会话
        session = await session_manager.create_session(sample_session_config)
        ...
        # 清理不活跃会话（24小时阈值）
        cleaned_count = await session_manager.cleanup_inactive_sessions(max_inactive_hours=24)
    ...
    async def test_search_sessions(self, session_manager):
        ...
        # 按状态搜索
        results = await session_manager.search_sessions({"status": SessionStatus.ACTIVE.value})
