...
class ReasoningPipeline:
    ...
    def __init__(self, llm_provider: Optional[LLMProvider] = None, provider_manager: Optional[ProviderManager] = None):
        """
        初始化推理流水线
        ...
        if provider_manager:
            ...
            logger.info(f"ReasoningPipeline initialized with single provider: {llm_provider.name}")
        else:
    ...
    async def process(self, context: ReasoningContext) -> ReasoningResult:
        ...
        elif hasattr(self, 'provider_manager') and self.provider_manager and self.provider_manager.default_provider:
        ...
        logger.info(f"Reasoning pipeline completed for session {context.session_id}, turn {context.turn_number}")
    ...
    async def _interpret_rules(self, context: ReasoningContext) -> InterpretationResult:
        ...
        # 创建临时Canon对象
        canon = MarkdownCanon(
            path=Path(f"session_{context.session_id}"),
            raw_content=context.rules_text
        )
...
请生成符合世界观规则的叙事响应。保持叙事一致性，尊重所有约束，并自然地推进故事发展。"""
    ...
    async def _check_consistency(self, context: ReasoningContext) -> Dict[str, Any]:
        ...
        return self.consistency_checker.check(
            ...
            constraints=context.interpretation_result.constraints if context.interpretation_result else []
        )
    
    def _generate_result(self, context: ReasoningContext, reasoning_steps: List[Dict]) -> ReasoningResult:
        """生成最终结果"""
        narrative_response = context.llm_response.content if context.llm_response else ""
        ...
        return ReasoningResult(
            ...
            metadata={
                ...
                "llm_model": context.llm_response.model if context.llm_response else "unknown",
                "consistency_passed": context.consistency_report.get("passed", False) if context.consistency_report else False
            }
        )
    ...
    def _calculate_confidence(self, context: ReasoningContext) -> float:
        ...
        # 基于一致性检查调整
        if context.consistency_report and context.consistency_report.get("passed", False):
            confidence += 0.2
    ...
    async def batch_process(self, contexts: List[ReasoningContext]) -> List[ReasoningResult]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                # 创建降级结果
                final_results.append(ReasoningResult(
    ...
    def filter_results(self, results: List[ReasoningResult], min_confidence: float = 0.5, min_quality: float = 0.6) -> List[ReasoningResult]:
        """过滤结果"""
        filtered = []
        ...
        for result in results:
            ...
            if result.confidence >= min_confidence and quality["overall_score"] >= min_quality:
                filtered.append(result)
            else:
                logger.warning(f"Filtered out result with confidence {result.confidence:.2f}, quality {quality['overall_score']:.2f}")
        
        return filtered
    
    async def process_with_retry(self, context: ReasoningContext, max_retries: int = 2) -> ReasoningResult:
        """带重试的处理"""
        best_result = None
        best_quality = 0.0
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if quality["overall_score"] >= 0.8:
                    logger.info(f"Attempt {attempt + 1}: Quality sufficient ({quality['overall_score']:.2f}), returning")
                    return result
                
                logger.info(f"Attempt {attempt + 1}: Quality {quality['overall_score']:.2f}, retrying...")
    ...
    def generate_report(self, results: List[ReasoningResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": sum(1 for r in results if r.confidence >= 0.5) / total * 100,
