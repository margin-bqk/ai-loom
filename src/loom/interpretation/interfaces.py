...
class InterpretationLayer(ABC):
    ...
    async def check_consistency(
        self, result: InterpretationResult, rules_text: str
    ) -> ConsistencyReport:
        """检查解释结果与规则的一致性
    ...
    @abstractmethod
    async def explain_reasoning(self, result: InterpretationResult) -> str:
        """解释推理过程
...
class RuleInterpreter(ABC):
    ...
    async def interpret_rules(
        self, rules_text: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """解释规则
    ...
    @abstractmethod
    async def extract_constraints(self, rules_text: str) -> List[Dict[str, Any]]:
        """提取约束条件
...
class ReasoningPipeline(ABC):
    ...
    @abstractmethod
    async def process(self, context: InterpretationContext) -> InterpretationResult:
        """处理推理流水线
...
class ConsistencyChecker(ABC):
    ...
    @abstractmethod
    async def check(
        self, narrative: str, rules_text: str, memories: List[str]
    ) -> ConsistencyReport:
        """检查一致性
    ...
    async def find_conflicts(
        self, narrative: str, rules_text: str
    ) -> List[Tuple[str, str]]:
        """查找冲突
