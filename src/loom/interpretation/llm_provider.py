...
class LLMProvider(ABC):
    ...
    async def generate_batch(self, prompts: List[str], **kwargs) -> List[LLMResponse]:
        ...
        try:
            ...
            for request in requests:
                try:
                    response = await self.generate(request.prompt, **request.extra_params)
                    results.append(response)
                except Exception as inner_e:
                    logger.error(f"Individual request failed: {inner_e}")
                    results.append(await self._generate_fallback(request.prompt, **request.extra_params))
            return results
    
    async def _generate_batch_impl(self, requests: List[LLMRequest]) -> List[LLMResponse]:
        """批量生成的具体实现（默认顺序处理）"""
        results = []
        for request in requests:
            results.append(await self._generate_impl(request.prompt, **request.extra_params))
        return results
    ...
    def validate_config(self) -> List[str]:
        ...
        # 检查必需字段（子类可以覆盖）
        if "api_key" not in self.config and self.provider_type not in ["ollama", "local"]:
...
class LLMProviderFactory:
    ...
    @staticmethod
    def create_from_configs(configs: Dict[str, Dict[str, Any]]) -> Dict[str, LLMProvider]:
        """从配置字典创建多个提供者"""
        providers = {}
