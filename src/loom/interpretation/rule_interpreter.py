...
class RuleInterpreter:
    ...
    def interpret(self, canon: MarkdownCanon, use_cache: bool = True) -> InterpretationResult:
        ...
        logger.info(f"Interpreted canon {canon.path}, found {len(constraints)} constraints")
    ...
    def _parse_permissions(self, content: str, section_name: str) -> List[RuleConstraint]:
        """解析权限章节"""
        constraints = []
        ...
        for line in lines:
            ...
            if any(word in line_lower for word in ["不能", "禁止", "不允许", "不可以"]):
                constraints.append(RuleConstraint(
                    type="permission",
                    content=line.strip(),
                    priority=3,  # 禁止性规则优先级较高
                    metadata={"is_prohibition": True}
                ))
            elif any(word in line_lower for word in ["可以", "允许", "能够"]):
                constraints.append(RuleConstraint(
                    type="permission",
                    content=line.strip(),
                    priority=2,
    ...
    def _extract_themes(self, section: CanonSection) -> List[str]:
        ...
        if section.section_type.name == "WORLD":
            world_themes = ["奇幻", "科幻", "现实", "历史", "神话", "未来", "古代"]
            themes.extend([t for t in world_themes if t in content])
        
        elif section.section_type.name == "TONE":
            tone_themes = ["黑暗", "光明", "幽默", "严肃", "浪漫", "恐怖", "冒险"]
    ...
    def _generate_summary(self, constraints: List[RuleConstraint], themes: List[str]) -> str:
        """生成摘要"""
        if not constraints:
            return "规则集未提供具体约束"
    ...
    def format_for_prompt(self, result: InterpretationResult, max_constraints: int = 20) -> str:
        """格式化解释结果以供Prompt使用"""
        sections = []
        ...
        if result.constraints:
            # 按优先级排序
            sorted_constraints = sorted(result.constraints, key=lambda c: c.priority, reverse=True)
            constraints_text = []
            
            for i, constraint in enumerate(sorted_constraints[:max_constraints]):
                priority_stars = "★" * constraint.priority
                constraints_text.append(f"{i+1}. [{constraint.type}] {constraint.content} {priority_stars}")
            
            sections.append(f"## 关键约束\n" + "\n".join(constraints_text))
        
        # 添加叙事指南
        if result.narrative_guidelines:
            guidelines_text = "\n".join([f"- {g}" for g in result.narrative_guidelines[:10]])
            sections.append(f"## 叙事指南\n{guidelines_text}")
        
        return "\n\n".join(sections)
    
    def detect_conflicts(self, constraints: List[RuleConstraint]) -> List[Dict[str, Any]]:
        """检测约束冲突"""
        conflicts = []
        
        # 检查权限冲突（允许 vs 禁止）
        permission_constraints = [c for c in constraints if c.type == "permission"]
        allowed = [c for c in permission_constraints if not c.metadata.get("is_prohibition", False)]
        prohibited = [c for c in permission_constraints if c.metadata.get("is_prohibition", False)]
        
        # 简单冲突检测：检查是否有相互矛盾的描述
        for allow in allowed:
            for prohibit in prohibited:
                # 简单关键词冲突检测
                allow_lower = allow.content.lower()
                prohibit_lower = prohibit.content.lower()
                
                # 提取动作关键词
                action_keywords = ["做", "使用", "说", "去", "改变", "创造", "破坏"]
                allow_actions = [kw for kw in action_keywords if kw in allow_lower]
                prohibit_actions = [kw for kw in action_keywords if kw in prohibit_lower]
                
                # 如果有共同的动作关键词，可能冲突
                common_actions = set(allow_actions) & set(prohibit_actions)
                if common_actions:
                    conflicts.append({
        ...
        time_flow_constraints = [c for c in causality_constraints if c.metadata.get("subtype") == "time_flow"]
        
        # 检查时间流向冲突（如同时允许和禁止时间倒流）
        time_keywords = ["时间倒流", "时间停止", "时间跳跃"]
        for keyword in time_keywords:
            related = [c for c in time_flow_constraints if keyword in c.content]
            if len(related) > 1:
                # 检查是否有矛盾描述
                contents = [c.content for c in related]
                if any("可以" in c for c in contents) and any("不能" in c for c in contents):
                    conflicts.append({
    ...
    def resolve_conflicts(self, constraints: List[RuleConstraint], conflicts: List[Dict[str, Any]]) -> List[RuleConstraint]:
        """解决约束冲突"""
        if not conflicts:
            return constraints
        ...
        for conflict in conflicts:
            if conflict["type"] == "permission_conflict":
                ...
                for constraint in resolved:
                    if constraint.content == constraint1 and constraint.metadata.get("is_prohibition", False):
                        # constraint1是禁止性规则，保留它
                        # 可能需要调整另一个规则的优先级
                        pass
                    elif constraint.content == constraint2 and constraint.metadata.get("is_prohibition", False):
    ...
    def get_contextual_rules(self, context: Dict[str, Any], result: InterpretationResult) -> List[RuleConstraint]:
        """获取上下文相关的规则"""
        contextual = []
        ...
        for constraint in result.constraints:
            ...
            # 根据上下文类型选择相关约束
            if context_type == "combat" and constraint.type in ["permissions", "conflict"]:
                contextual.append(constraint)
            elif context_type == "dialogue" and constraint.type in ["tone", "permissions"]:
                contextual.append(constraint)
            elif context_type == "world_building" and constraint.type in ["world", "causality"]:
                contextual.append(constraint)
