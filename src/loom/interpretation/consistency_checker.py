...
class ConsistencyChecker:
    ...
    def check(self, response: str, rules_text: str, constraints: List[RuleConstraint]) -> Dict[str, Any]:
        """检查一致性"""
        issues = []
    ...
    def _check_rule_violations(self, response: str, constraints: List[RuleConstraint]) -> List[ConsistencyIssue]:
        """检查规则违反"""
        issues = []
        ...
        for constraint in constraints:
            if constraint.type == "permission":
                ...
                if constraint.metadata.get("is_prohibition", False):
                    # 简单关键词匹配
                    violation = self._check_prohibition_violation(response, constraint.content)
                    if violation:
                ...
                # 检查因果关系违反
                violation = self._check_causality_violation(response, constraint.content)
                if violation:
                    issues.append(ConsistencyIssue(
                        type="rule_violation",
                        severity="medium",
    ...
    def _check_tone_consistency(self, response: str, rules_text: str) -> List[ConsistencyIssue]:
        """检查基调一致性"""
        issues = []
    ...
    def check_with_llm(self, response: str, rules_text: str, llm_provider) -> Dict[str, Any]:
        """使用LLM进行更深入的一致性检查"""
        # 此方法可以使用LLM进行更复杂的检查
        # 简化实现
    ...
    def check_with_memories(self, response: str, memories: List[Dict[str, Any]], constraints: List[RuleConstraint] = None) -> Dict[str, Any]:
        """检查与历史记忆的一致性"""
        issues = []
        ...
        logger.debug(f"Memory consistency check: {len(issues)} issues, score={score:.2f}, memories={len(memories)}")
        
        return report.__dict__
    
    def _check_memory_consistency(self, response: str, memories: List[Dict[str, Any]]) -> List[ConsistencyIssue]:
        """检查记忆一致性"""
        issues = []
    ...
    def _check_fact_consistency(self, response: str, fact: Dict[str, Any]) -> List[ConsistencyIssue]:
        """检查事实一致性"""
        issues = []
    ...
    def _check_character_consistency(self, response: str, character: Dict[str, Any]) -> List[ConsistencyIssue]:
        """检查角色一致性"""
        issues = []
    ...
    def _check_event_consistency(self, response: str, event: Dict[str, Any]) -> List[ConsistencyIssue]:
        """检查事件一致性"""
        issues = []
    ...
    def _generate_memory_summary(self, issues: List[ConsistencyIssue], score: float, memory_count: int) -> str:
        """生成记忆一致性摘要"""
        if not issues:
            return f"记忆一致性检查通过（检查了{memory_count}条记忆），分数：{score:.2f}"
        
        memory_issues = [i for i in issues if i.type in ["memory_conflict", "character_inconsistency"]]
        other_issues = [i for i in issues if i.type not in ["memory_conflict", "character_inconsistency"]]
    ...
    def generate_correction_suggestions(self, issues: List[ConsistencyIssue], response: str) -> List[Dict[str, str]]:
        """生成修正建议"""
        suggestions = []
        ...
        for issue in issues:
            suggestion = {
                ...
                "suggestion": issue.suggestion or self._get_default_suggestion(issue.type),
