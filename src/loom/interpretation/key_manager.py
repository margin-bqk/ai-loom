...
class KeyManager:
            ...
            try:
                ...
                for key_data in data.get("keys", []):
                    try:
                        ...
                        logger.error(f"Failed to load key {key_data.get('key_id', 'unknown')}: {e}")
    ...
    def _save_keys(self):
        ...
        try:
            data = {
                "keys": [key_info.to_dict(mask=False) for key_info in self._keys.values()],
    ...
    def add_key(self, provider: str, key_value: str, key_type: str = "api_key", 
    ...
    def get_key_info(self, provider: str, key_id: Optional[str] = None) -> Optional[APIKeyInfo]:
        """获取密钥信息"""
        for key_info in self._keys.values():
            if key_info.provider == provider and key_info.is_active:
                if key_id is None or key_info.key_id == key_id:
    ...
    def rotate_key(self, provider: str, new_key_value: str) -> str:
        ...
        self._save_keys()
        logger.info(f"Rotated key for provider {provider}, new key ID: {new_key_id[:8]}...")
        return new_key_id
    
    def validate_key(self, provider: str, key_value: Optional[str] = None) -> Dict[str, Any]:
        """验证密钥有效性"""
    ...
    def export_keys(self, include_values: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            "warning": "Keep this data secure!" if include_values else "Keys are masked for security"
        }
