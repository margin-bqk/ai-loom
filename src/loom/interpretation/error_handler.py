    ...
    def should_allow_request(self, reset_timeout: float = 60.0) -> bool:
        ...
        if self.opened_at:
            ...
            if elapsed >= reset_timeout:
                # 尝试半开状态
                self.is_open = False
                logger.info(f"Circuit breaker transitioning to half-open after {elapsed:.1f}s")
                return True
...
class ErrorHandler:
    ...
    def get_retry_delay(self, error_info: ErrorInfo, retry_policy: RetryPolicy) -> float:
        """获取重试延迟"""
    ...
    def get_provider_health(self, provider: str) -> Dict[str, Any]:
        ...
        if provider in self.circuit_breakers:
            ...
            if circuit_breaker.opened_at:
                health["circuit_breaker_opened_at"] = circuit_breaker.opened_at.isoformat()
        
        return health
    
    def get_best_provider(self, providers: List[str], exclude: Optional[List[str]] = None) -> Optional[str]:
        """获取最佳Provider（基于健康状态）"""
        if exclude is None:
            exclude = []
        ...
        logger.debug(f"Selected best provider: {best_provider} with score {provider_scores[best_provider]:.1f}")
    ...
    def classify_error(self, error: Exception, provider: str) -> ErrorInfo:
        ...
        if isinstance(error, TimeoutError) or "timeout" in error_message.lower():
            ...
            error_severity = ErrorSeverity.MEDIUM
        elif isinstance(error, ConnectionError) or "connection" in error_message.lower():
            ...
            error_severity = ErrorSeverity.MEDIUM
        elif "quota" in error_message.lower() or "insufficient credits" in error_message.lower():
            error_category = ErrorCategory.QUOTA_EXCEEDED
            error_severity = ErrorSeverity.HIGH
        elif "model" in error_message.lower() and "unavailable" in error_message.lower():
            ...
            error_severity = ErrorSeverity.HIGH
        elif "server error" in error_message.lower() or "internal error" in error_message.lower():
...
class FallbackStrategy:
    ...
    async def execute_fallback(self, original_request: Dict[str, Any], 
                              error: ErrorInfo, 
        ...
        if error.severity == ErrorSeverity.LOW:
            ...
            strategies_to_try = ["provider_switch", "model_downgrade", "response_caching"]
        ...
        for strategy_name in strategies_to_try:
            try:
                ...
                result = await strategy_func(original_request, error, available_providers)
                if result:
    ...
    async def _model_downgrade(self, request: Dict[str, Any], 
                              error: ErrorInfo, 
    ...
    async def _provider_switch(self, request: Dict[str, Any], 
                              error: ErrorInfo, 
                              available_providers: List[str]) -> Optional[Dict[str, Any]]:
        """Provider切换"""
        # 排除当前失败的Provider
        ...
        # 获取最佳Provider
        best_provider = self.error_handler.get_best_provider(available_providers, exclude_providers)
    ...
    async def _simplified_response(self, request: Dict[str, Any],
                                  error: ErrorInfo,
                                  available_providers: List[str]) -> Optional[Dict[str, Any]]:
        """简化响应"""
        # 生成一个简化的响应
    ...
    for attempt in range(retry_policy.max_retries + 1):
        try:
            if attempt > 0:
                ...
                logger.info(f"Retry attempt {attempt}/{retry_policy.max_retries} for {provider}, waiting {delay:.1f}s")
