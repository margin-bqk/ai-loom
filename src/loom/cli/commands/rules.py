    ...
    try:
        ...
        if output_format == "json":
            ...
            for name, canon in canons.items():
                ...
                typer.echo(
                    f"{name},{str(canon.path)},{sections_count},{metadata_count}"
                )
        else:  # table
            from rich.console import Console
            from rich.table import Table
...
def create_rules(
    ...
    template: str = typer.Option(
        ...
        "--template",
        "-t",
        help="模板类型 (default, fantasy, scifi, minimal)",
    ),
    force: bool = typer.Option(False, "--force", "-f", help="强制覆盖现有文件"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="详细输出"),
):
    """创建新的规则集"""
        ...
        if output_format == "json":
            ...
            if stats.get("available_files"):
                ...
                if len(stats["available_files"]) > 10:
                    files_table.add_row(
                        "...",
                        f"还有 {len(stats['available_files']) - 10} 个文件",
                        "",
                        "",
                    )
