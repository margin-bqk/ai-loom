...
@app.command("lint")
def lint_code(
    path: str = typer.Option(".", "--path", "-p", help="检查路径"),
...
@app.command("test")
def run_tests(
    path: str = typer.Option("tests", "--path", "-p", help="测试路径"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="详细输出"),
    coverage: bool = typer.Option(False, "--coverage", "-c", help="生成覆盖率报告"),
    markers: Optional[str] = typer.Option(None, "--markers", "-m", help="测试标记过滤"),
):
    """运行测试"""
    typer.echo("运行测试...")
...
@app.command("build")
def build_package(
    clean: bool = typer.Option(False, "--clean", "-c", help="清理构建目录"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="详细输出"),
):
    """构建包"""
    typer.echo("构建包...")
...
@app.command("docs")
def build_docs(
    clean: bool = typer.Option(False, "--clean", "-c", help="清理文档构建目录"),
    serve: bool = typer.Option(False, "--serve", "-s", help="构建后启动本地服务器"),
...
def check_dependencies(
    ...
    if python_version.major == 3 and python_version.minor >= 10:
        typer.echo(
            f"✅ Python {python_version.major}.{python_version.minor}.{python_version.micro} 符合要求 (>=3.10)"
        )
    else:
        typer.echo(
            f"❌ Python 版本 {python_version.major}.{python_version.minor} 不符合要求 (需要 >=3.10)",
            err=True,
        )
        checks_passed = False
...
@app.command("clean")
def clean_project(
    all: bool = typer.Option(False, "--all", "-a", help="清理所有生成文件"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="详细输出"),
):
    """清理项目生成文件"""
    typer.echo("清理项目生成文件...")
