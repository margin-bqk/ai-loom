...
def create_session(
    ...
    # 异步运行
    asyncio.run(_create_session_async(name, canon, provider, max_turns, output))
...
async def _list_sessions_async(
    ...
    try:
        ...
        if output_format == "json":
            ...
            typer.echo(
                "session_id,name,status,current_turn,total_turns,created_at,llm_provider"
            )
            for session_id, session in sessions.items():
                typer.echo(
                    f"{session_id},{session.name},{session.status.value},{session.current_turn},{session.total_turns},{session.created_at},{session.config.llm_provider}"
                )
        else:  # table
...
async def _show_session_async(session_id: str, output_format: str):
    ...
    try:
        ...
        if output_format == "json":
            result = {"session": session.to_dict(), "stats": stats}
            ...
            info_table.add_row(
                "最大回合数",
                str(session.config.max_turns) if session.config.max_turns else "无限制",
            )
...
def update_session(
    session_id: str = typer.Argument(..., help="会话ID"),
    name: Optional[str] = typer.Option(None, "--name", "-n", help="更新会话名称"),
    status: Optional[str] = typer.Option(
        None,
        "--status",
        "-s",
        help="更新会话状态 (active, paused, completed, archived, error)",
    ),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="详细输出"),
):
    """更新会话属性"""
...
def cleanup_sessions(
    max_hours: int = typer.Option(24, "--max-hours", "-h", help="最大不活跃小时数"),
    dry_run: bool = typer.Option(False, "--dry-run", "-d", help="模拟运行，不实际删除"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="详细输出"),
):
    """清理不活跃的会话"""
