...
class OOCHandler:
    ...
    def __init__(self):
        self.ooc_patterns = [
            # 括号格式
            re.compile(r'\(OOC:\s*(.*?)\)', re.IGNORECASE | re.DOTALL),
            re.compile(r'\[OOC:\s*(.*?)\]', re.IGNORECASE | re.DOTALL),
            re.compile(r'\{OOC:\s*(.*?)\}', re.IGNORECASE | re.DOTALL),
            # 斜杠格式
    ...
    def categorize_ooc(self, ooc_comments: List[OOCComment]) -> Dict[str, List[OOCComment]]:
        """对OOC注释进行分类"""
        categories = {
            "clarification": [],  # 澄清说明
            "intent": [],         # 意图声明
    ...
    async def process_ooc_for_session(self, session_id: str, ooc_comments: List[OOCComment]) -> Dict[str, Any]:
        """为会话处理OOC注释"""
        # 分类OOC注释
        ...
        logger.info(f"Processed {len(ooc_comments)} OOC comments for session {session_id}")
        
        return result
    
    def merge_ooc_back(self, narrative_text: str, ooc_comments: List[OOCComment], style: str = "footnote") -> str:
        """将OOC注释合并回文本"""
        if not ooc_comments:
    ...
    def should_respond_to_ooc(self, ooc_comments: List[OOCComment]) -> bool:
        ...
        for comment in ooc_comments:
            ...
            # 如果包含问号或明确请求回应
            if "?" in comment.content or any(word in content_lower for word in ["回答", "回应", "请回复"]):
                return True
    ...
    def extract_intents_for_prompt(self, ooc_comments: List[OOCComment]) -> Dict[str, Any]:
    ...
    def prepare_ooc_for_prompt_assembler(self, ooc_comments: List[OOCComment]) -> List[Dict[str, Any]]:
        ...
        for comment in ooc_comments:
            ...
            primary_category = next((cat for cat, comments in categories.items() if comments), "other")
            ...
            if primary_category == "intent":
                ...
                intervention["question_type"] = self._classify_question_type(comment.content)
            
            interventions.append(intervention)
        
        return interventions
    
    async def integrate_with_prompt_assembler(self, prompt_assembler, context, ooc_comments: List[OOCComment]) -> Dict[str, Any]:
        """与PromptAssembler集成"""
