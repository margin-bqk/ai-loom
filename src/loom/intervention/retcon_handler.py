...
class RetconHandler:
    ...
    def parse_retcon(self, retcon_text: str) -> Optional[RetconOperation]:
        """解析Retcon命令（parse_retcon_command的别名）"""
        return self.parse_retcon_command(retcon_text)
    ...
    async def execute_retcon(
        self, operation: RetconOperation, session_context: Dict[str, Any]
    ) -> RetconResult:
        """执行Retcon操作"""
        try:
            # 创建版本快照
            version_id = await self._create_snapshot(
            ...
            logger.info(
                f"Executed retcon: {operation.type} - Success: {result.success}"
            )
    ...
    async def _handle_modify_fact(
        ...
        new_content = operation.changes.get(
            "new", operation.changes.get("new_value", "")
        )
    ...
    async def _handle_remove_memory(
        self, operation: RetconOperation, session_context: Dict[str, Any]
    ) -> RetconResult:
        """移除记忆"""
        if not self.world_memory or not operation.target_id:
    ...
    async def _handle_alter_timeline(
        self, operation: RetconOperation, session_context: Dict[str, Any]
    ) -> RetconResult:
        """修改时间线"""
        # 时间线修改是复杂操作，这里简化处理
    ...
    async def _check_consistency(
        self, operation: RetconOperation, result: RetconResult
    ) -> List[str]:
        """检查一致性"""
        issues = []
        ...
        if operation.type == "modify_fact":
            if (
                "->" not in operation.metadata.get("raw_text", "")
                and "new_value" not in operation.changes
            ):
    ...
    async def validate_retcon(
        self, operation: RetconOperation, rules_text: str
    ) -> Tuple[bool, List[str]]:
        """验证Retcon是否允许"""
        errors = []
        ...
        if "禁止修改历史" in rules_text and operation.type in [
            "modify_fact",
            "alter_timeline",
        ]:
    ...
    async def check_retcon_consistency(
        self,
        operation: RetconOperation,
        consistency_checker,
        rules_text: str,
        ...
        try:
            ...
            if memories:
                ...
                result = consistency_checker.check_with_memories(
                    simulated_response, memories, constraints=[]
                )
            else:
                # 使用规则检查
                result = consistency_checker.check(
                    simulated_response, rules_text, constraints=[]
                )
            ...
            # 提取关键问题
            critical_issues = [
                issue for issue in issues if issue.get("severity") in ["high", "medium"]
            ]
    ...
    def _generate_consistency_recommendation(
        self, score: float, critical_issues: List[Dict[str, Any]]
    ) -> str:
        """生成一致性建议"""
        if score >= 0.8:
    ...
    async def resolve_retcon_conflicts(
        self,
        operation: RetconOperation,
        ...
        for conflict in conflicts:
            ...
            if conflict_type == "memory_conflict" and world_memory:
                # 尝试解决记忆冲突
                resolution = await self._resolve_memory_conflict(
                    operation, conflict, world_memory
                )
                if resolution["success"]:
                    resolved_conflicts.append({**conflict, "resolution": resolution})
                else:
                    unresolved_conflicts.append({**conflict, "resolution_failed": True})
            else:
                # 标记为需要手动解决
        ...
        return {
            ...
            "recommendation": self._generate_conflict_resolution_recommendation(
        ...
        try:
            ...
            if operation.type == "modify_fact":
                ...
                if updated_entity:
                    ...
                    return {
                        "success": False,
                        "error": "Failed to mark memory as removed",
                    }

            else:
                return {
                    "success": False,
                    "error": f"Unsupported operation type for conflict resolution: {operation.type}",
                }
    ...
    def _generate_conflict_resolution_recommendation(
        self, resolved: List[Dict[str, Any]], unresolved: List[Dict[str, Any]]
    ) -> str:
        """生成冲突解决建议"""
        if not unresolved:
            return "所有冲突已成功解决"
    ...
    async def execute_retcon_with_consistency_check(
        self,
        operation: RetconOperation,
        ...
        if not consistency_check["success"]:
            return RetconResult(
                ...
                consistency_issues=[
                    "一致性检查失败: " + consistency_check.get("error", "未知错误")
                ],
    ...
    async def integrate_with_world_memory(
        self,
        world_memory: Optional[WorldMemory] = None,
        operations: Optional[List[RetconOperation]] = None,
    ) -> Dict[str, Any]:
        """与WorldMemory集成"""
        ...
        try:
            ...
            if operations:
                for operation in operations:
                    if operation.type == "alter_timeline":
                        # 更新时间线
                        success = await memory.update_timeline(
                            {
                                "event": operation.changes.get("event", "未知事件"),
                                "new_time": operation.changes.get("new_time", "新时间"),
                                "justification": operation.justification,
                        ...
                        if success:
                            timeline_updated += 1
                    elif operation.type == "modify_fact":
                        # 更新事实
                        success = await memory.update_fact(
                            {
                                "old": operation.changes.get("old", "原事实"),
                                "new": operation.changes.get("new", "新事实"),
                                "justification": operation.justification,
                            }
                        )
                        if success:
