...
class WorldEditor:
    ...
    def __init__(self, world_memory: Optional[WorldMemory] = None, rule_loader: Optional[RuleLoader] = None):
    ...
    def parse_edit_command(self, edit_text: str) -> Optional[EditCommand]:
        ...
        logger.debug(f"Parsed edit command: {target_type}.{action}")
        return command
    ...
    def _parse_parameters(self, param_text: str) -> Dict[str, Any]:
        ...
        if not parameters and param_text:
            parameters["value"] = param_text
        ...
        try:
            ...
            if not handler:
                return EditResult(
                    ...
                    errors=[f"Unsupported operation: {command.target_type}.{command.action}"]
                )
            
            result = await handler(command, session_context)
            self.edit_history.append(result)
            
            logger.info(f"Executed edit: {command.target_type}.{command.action} - Success: {result.success}")
    ...
    async def _handle_character_add(self, command: EditCommand, session_context: Dict[str, Any]) -> EditResult:
        """添加角色"""
        ...
        if success:
            return EditResult(
                success=True,
                command=command,
                changes_made=[{"action": "add_character", "entity_id": character_id, "name": character_name}],
    ...
    async def _handle_character_update(self, command: EditCommand, session_context: Dict[str, Any]) -> EditResult:
        """更新角色"""
        ...
        # 更新实体
        updated_entity = await self.world_memory.update_entity(command.target_id, updates)
        
        if updated_entity:
            return EditResult(
                success=True,
                command=command,
                changes_made=[{"action": "update_character", "entity_id": command.target_id, "updates": updates}],
    ...
    async def _handle_fact_add(self, command: EditCommand, session_context: Dict[str, Any]) -> EditResult:
        """添加事实"""
        if not self.world_memory:
            return EditResult(
                success=False,
        ...
        fact_content = command.parameters.get("value", command.parameters.get("content", "新事实"))
        ...
        if success:
            return EditResult(
                success=True,
                command=command,
                changes_made=[{"action": "add_fact", "entity_id": fact_id, "content": fact_content}],
    ...
    async def _handle_relation_add(self, command: EditCommand, session_context: Dict[str, Any]) -> EditResult:
        """添加关系"""
        if not self.world_memory:
            return EditResult(
                success=False,
        ...
        if success:
            return EditResult(
                success=True,
                command=command,
                changes_made=[{"action": "add_relation", "source": source_id, "target": target_id, "type": relation_type.value}],
    ...
    async def validate_edit(self, command: EditCommand, rules_text: str) -> Tuple[bool, List[str]]:
        ...
        if "禁止添加角色" in rules_text and command.target_type == "character" and command.action == "add":
            errors.append("规则禁止添加角色")
        
        if "禁止修改事实" in rules_text and command.target_type == "fact" and command.action == "update":
    ...
    async def undo_last_edit(self) -> Optional[EditResult]:
        ...
        undo_result = EditResult(
            ...
            changes_made=[{"action": "undo", "original_edit": last_edit.command.metadata.get("raw_text", "")}],
            narrative_impact="撤销了最后一次编辑",
            warnings=["撤销操作可能不完全恢复原始状态"]
        )
        
        self.edit_history.append(undo_result)
        logger.info(f"Undid last edit: {last_edit.command.target_type}.{last_edit.command.action}")
        
        return undo_result
    
    # 规则修改方法
    
    async def modify_rule(self, rule_path: str, section: str, new_content: str, justification: str = "玩家编辑") -> EditResult:
        """修改规则文件"""
    ...
    async def create_rule(self, rule_path: str, content: str, justification: str = "玩家创建") -> EditResult:
        """创建新规则文件"""
        if not self.rule_loader:
            return EditResult(
                success=False,
    ...
    async def integrate_with_rule_loader(self, rule_loader: Optional[RuleLoader] = None) -> Dict[str, Any]:
        """与RuleLoader集成"""
    ...
    async def integrate_with_world_memory(self, world_memory: Optional[WorldMemory] = None) -> Dict[str, Any]:
        """与WorldMemory集成"""
    ...
    async def execute_comprehensive_edit(self, edit_text: str, session_context: Dict[str, Any]) -> EditResult:
        """执行综合编辑（支持规则和世界编辑）"""
        # 解析编辑命令
        ...
        if command.target_type == "rule":
            ...
            content = command.parameters.get("value", command.parameters.get("content", ""))
            
            if command.action == "modify":
                return await self.modify_rule(
                    command.target_id or "default",
                    section,
                    content,
                    "玩家编辑"
                )
