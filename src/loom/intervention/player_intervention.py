...
class PlayerIntervention:
    ...
    def _initialize_patterns(self) -> Dict[InterventionType, re.Pattern]:
        """初始化干预模式"""
        return {
            InterventionType.OOC: re.compile(r'\(OOC:\s*(.*?)\)', re.IGNORECASE | re.DOTALL),
            InterventionType.WORLD_EDIT: re.compile(r'\[EDIT:\s*(.*?)\]', re.IGNORECASE | re.DOTALL),
            InterventionType.RETCON: re.compile(r'\[RETCON:\s*(.*?)\]', re.IGNORECASE | re.DOTALL),
            InterventionType.TONE_ADJUST: re.compile(r'\[TONE:\s*(.*?)\]', re.IGNORECASE | re.DOTALL),
            InterventionType.INTENT_DECLARE: re.compile(r'\[INTENT:\s*(.*?)\]', re.IGNORECASE | re.DOTALL),
            InterventionType.META_COMMENT: re.compile(r'\[META:\s*(.*?)\]', re.IGNORECASE | re.DOTALL),
        }
    ...
    def _initialize_conflict_rules(self) -> List[Tuple[InterventionType, InterventionType, str]]:
        """初始化冲突规则"""
        return [
            (InterventionType.RETCON, InterventionType.WORLD_EDIT, "Retcon和世界编辑可能冲突"),
            (InterventionType.WORLD_EDIT, InterventionType.RETCON, "世界编辑和Retcon可能冲突"),
            (InterventionType.TONE_ADJUST, InterventionType.INTENT_DECLARE, "基调调整和意图声明可能冲突"),
        ]
    ...
    def parse_input(self, player_input: str) -> Dict[str, Any]:
        ...
        for interv_type, pattern in self.patterns.items():
            ...
            for match in matches:
                ...
                # 设置优先级
                priority = self.priority_map.get(interv_type, InterventionPriority.MEDIUM)
        ...
        logger.debug(f"Parsed input: found {len(interventions)} interventions, {len(conflicts)} conflicts")
        for interv in interventions:
            logger.debug(f"  - {interv.type.value} (priority={interv.priority.name}): {interv.content[:50]}...")
    ...
    def _detect_conflicts(self, interventions: List[Intervention]) -> List[Dict[str, Any]]:
        """检测干预之间的冲突"""
        conflicts = []
    ...
    def prioritize_interventions(self, interventions: List[Intervention]) -> List[Intervention]:
        """按优先级排序干预"""
    ...
    async def process_interventions(self, interventions: List[Intervention], session_context: Dict[str, Any]) -> List[InterventionResult]:
        ...
        for intervention in prioritized:
            result = await self._process_single_intervention(intervention, session_context)
            results.append(result)
            
            logger.info(f"Processed {intervention.type.value} intervention (priority={intervention.priority.name}): {result.success}")
            if not result.success:
                logger.warning(f"Intervention failed: {result.errors}")
        
        return results
    
    async def _process_single_intervention(self, intervention: Intervention, session_context: Dict[str, Any]) -> InterventionResult:
        """处理单个干预"""
        try:
            ...
            if not handler_method:
                return InterventionResult(
                    ...
                    actions_taken=[],
                    errors=[f"Unsupported intervention type: {intervention.type.value}"]
                )
    ...
    async def _handle_ooc(self, intervention: Intervention, session_context: Dict[str, Any]) -> InterventionResult:
        """处理OOC注释"""
        # OOC注释不影响叙事，仅记录
        return InterventionResult(
            success=True,
            ...
            actions_taken=[
                {"action": "log_ooc", "content": intervention.content}
            ],
            warnings=["OOC注释将被记录但不影响叙事"]
        )
    
    async def _handle_world_edit(self, intervention: Intervention, session_context: Dict[str, Any]) -> InterventionResult:
        """处理世界编辑"""
        # 解析编辑指令
    ...
    async def _handle_retcon(self, intervention: Intervention, session_context: Dict[str, Any]) -> InterventionResult:
        """处理Retcon"""
        # Retcon需要更复杂的处理
        return InterventionResult(
            success=True,
            ...
            actions_taken=[
                {"action": "retcon", "content": intervention.content}
            ],
            warnings=["Retcon可能影响叙事一致性，请谨慎使用"]
        )
    
    async def _handle_tone_adjust(self, intervention: Intervention, session_context: Dict[str, Any]) -> InterventionResult:
        """处理基调调整"""
        ...
        return InterventionResult(
            ...
            actions_taken=[
                {"action": "tone_adjust", "new_tone": tone_change}
            ]
        )
    
    async def _handle_intent_declare(self, intervention: Intervention, session_context: Dict[str, Any]) -> InterventionResult:
        """处理意图声明"""
        ...
        return InterventionResult(
            ...
            actions_taken=[
                {"action": "intent_declare", "intent": intent}
            ]
        )
    
    async def _handle_meta_comment(self, intervention: Intervention, session_context: Dict[str, Any]) -> InterventionResult:
        """处理元注释"""
        return InterventionResult(
            success=True,
            intervention=intervention,
            ...
            actions_taken=[
                {"action": "log_meta", "content": intervention.content}
            ]
        )
    ...
    def merge_interventions_into_prompt(self, clean_input: str, intervention_results: List[InterventionResult]) -> str:
        ...
        for result in intervention_results:
            ...
            intervention_texts.append(f"[{interv.type.value.upper()}: {interv.content}]")
    ...
    async def audit_intervention(self, intervention: Intervention, result: InterventionResult, user_id: str = "unknown") -> Dict[str, Any]:
        """审计干预"""
        audit_record = {
        ...
        logger.info(f"Audit: {intervention.type.value} intervention by {user_id} - Success: {result.success}")
        
        return audit_record
    
    # 与SessionManager和TurnScheduler集成的方法
    
    async def integrate_with_session(self, session_manager, session_id: str, interventions: List[Intervention]) -> Dict[str, Any]:
        ...
        # 更新会话元数据
        session.metadata["last_intervention_time"] = __import__('datetime').datetime.now().isoformat()
        session.metadata["total_interventions"] = session.metadata.get("total_interventions", 0) + len(interventions)
        ...
        return {
            ...
            "session_stats": {
                ...
                "last_intervention_time": session.metadata.get("last_intervention_time")
            }
        }
    
    async def integrate_with_turn_scheduler(self, turn_scheduler, turn_id: str, intervention_results: List[InterventionResult]) -> Dict[str, Any]:
        ...
        # 更新回合优先级（如果有高优先级干预）
        max_priority = max([result.intervention.priority.value for result in intervention_results], default=0)
        if max_priority > turn.priority:
            turn.priority = max_priority
            logger.info(f"Updated turn {turn_id} priority to {turn.priority} based on interventions")
    ...
    async def process_player_input_with_integration(
        ...
        intervention_results = await self.process_interventions(parsed["interventions"], session_context)
