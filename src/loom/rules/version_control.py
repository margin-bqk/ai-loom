...
class VersionControl:
    ...
    def __init__(self, repo_path: str = "./canon"):
        ...
        logger.info(f"VersionControl initialized for {repo_path} (enabled={self.enabled})")
    ...
    def integrate_with_rule_loader(self, rule_loader: RuleLoader) -> Dict[str, Any]:
        ...
        if status.get("has_changes", False):
            logger.info(f"Auto-committing {len(status.get('changed_files', []))} changed files")
    ...
    def get_canon_diff(self, canon_name: str, commit1: str = "HEAD~1", commit2: str = "HEAD") -> Optional[Dict[str, Any]]:
        """获取规则文件的差异
        ...
        # 获取差异
        success, diff = self._run_git_command(["diff", commit1, commit2, "--", file_path])
        if not success:
            return None
    ...
    def create_version_snapshot(self, version_name: str, rule_loader: RuleLoader) -> bool:
        """创建版本快照
        ...
        # 创建标签
        success = self.tag_version(version_name, f"LOOM version snapshot: {version_name}")
    ...
    def compare_versions(self, version1: str, version2: str) -> Dict[str, Any]:
        ...
        # 获取差异统计
        success, diff_stat = self._run_git_command(["diff", "--stat", version1, version2])
        if not success:
            return {"error": f"Failed to compare {version1} and {version2}"}
        
        # 获取更改的文件列表
        success, name_status = self._run_git_command(["diff", "--name-status", version1, version2])
        changed_files = []
