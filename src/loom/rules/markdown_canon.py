...
class MarkdownCanon:
    ...
    def _parse_content(self):
        ...
        for i in range(frontmatter_end, len(lines)):
            ...
            if line.startswith('# '):
                # 保存前一个章节
                if current_section:
                    self._add_section(current_section, section_content, section_start, i-1)
        ...
        # 保存最后一个章节
        if current_section:
            self._add_section(current_section, section_content, section_start, len(lines)-1)
                ...
                try:
                    ...
                    if frontmatter_text.strip():
                        ...
                        if frontmatter_data and isinstance(frontmatter_data, dict):
                            ...
                            logger.debug(f"Extracted frontmatter metadata: {list(frontmatter_data.keys())}")
    ...
    def search_content(self, query: str, section_type: Optional[CanonSectionType] = None) -> List[Tuple[str, str]]:
        """搜索规则内容
    ...
    def validate_with_schema(self, schema: Optional[Dict[str, Any]] = None) -> List[str]:
        """使用模式验证规则完整性
