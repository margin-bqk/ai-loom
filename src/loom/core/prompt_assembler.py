...
请根据玩家的输入和以上上下文，生成符合规则的叙事响应。""",
    ...
    def assemble(self, context: PromptContext) -> PromptResult:
        ...
        if template_name not in self.templates:
            ...
            logger.warning(
                f"Template '{context.system_prompt_template}' not found, using 'default'"
            )
        ...
        messages = self._build_messages(
            system_prompt, user_prompt, context.llm_provider
        )
        ...
        if context.max_tokens and token_estimate > context.max_tokens:
            logger.warning(
                f"Prompt token estimate ({token_estimate}) exceeds limit ({context.max_tokens})"
            )
            # 可以在这里实现截断逻辑
        ...
        logger.debug(
            f"Assembled prompt for session {context.session_id}, turn {context.turn_number}"
        )
        logger.debug(
            f"Template: {template_name}, Tokens: {token_estimate}, Memories: {len(context.memories)}"
        )

        return result

    def _prepare_variables(
        self, context: PromptContext, template_name: str
    ) -> Dict[str, str]:
        """准备模板变量"""
        from datetime import datetime
    ...
    def _summarize_rules(self, rules_text: str) -> str:
        ...
        # 简单截断，可以改进为智能摘要
        summary = (
            rules_text[: self.template_variables["max_rules_length"]]
            + "...\n\n（规则过长，已截断）"
        )
    ...
    def _format_memories(
        self, memories: List[Dict[str, Any]], format_type: str = "brief"
    ) -> str:
        """格式化记忆"""
        if not memories:
            return "（无相关记忆）"
    ...
    def _combine_input_and_interventions(
        self, player_input: str, interventions: List[Dict[str, Any]]
    ) -> str:
        """合并玩家输入和干预信息"""
        if not interventions:
            return player_input
...
4. 新信息应与已有记忆协调"""
    ...
    def _build_messages(
        self, system_prompt: str, user_prompt: str, llm_provider: str
    ) -> List[Dict[str, str]]:
        """构建消息列表（适用于Chat API）"""
        messages = []
    ...
    def truncate_to_fit_tokens(
        self, context: PromptContext, max_tokens: int
    ) -> PromptContext:
        """截断内容以适应令牌限制"""
        # 创建副本以避免修改原始上下文
        ...
        if truncated_context.memories:
            # 减少记忆数量
            target_memory_count = max(
                1, len(truncated_context.memories) - (excess_tokens // 50)
            )
            truncated_context.memories = truncated_context.memories[
                :target_memory_count
            ]
        ...
        if len(truncated_context.rules_text) > 1000:
            truncated_context.rules_text = (
                truncated_context.rules_text[:1000] + "...\n（规则已截断）"
            )

        # 3. 截断玩家输入（最后手段）
        if len(truncated_context.player_input) > 500:
            truncated_context.player_input = (
                truncated_context.player_input[:500] + "..."
            )
        ...
        return PromptContext(
            ...
            additional_context={
                ...
                "session_config": (
                    ...
                    if hasattr(session.config, "__dict__")
                    else session.config
                ),
            },
        )
