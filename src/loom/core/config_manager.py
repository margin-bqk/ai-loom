...
class ProviderSelectionConfig(BaseModel):
    ...
    fallback_order: List[str] = Field(default_factory=lambda: ["openai", "anthropic", "ollama"])
    
    # 基于会话类型的Provider选择
    session_type_mapping: Dict[str, Dict[str, Any]] = Field(default_factory=dict)
    
    # 自动切换策略
    auto_switch: Dict[str, Any] = Field(default_factory=lambda: {
        "enabled": True,
...
class AppConfig:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], config_path: Optional[str] = None) -> 'AppConfig':
        """从字典创建配置，支持环境变量插值"""
        # 处理环境变量插值
        ...
        if "provider_selection" in data:
            try:
                config.provider_selection = ProviderSelectionConfig(**data["provider_selection"])
            except ValidationError as e:
        ...
        if "session_defaults" in data:
            try:
                config.session_defaults = SessionDefaultsConfig(**data["session_defaults"])
            except ValidationError as e:
        ...
        # 处理其他配置
        if "max_concurrent_turns" in data:
            config.max_concurrent_turns = data["max_concurrent_turns"]
...
class ConfigManager:
    ...
    def _merge_env_overrides(self, data: Dict[str, Any]):
        ...
        for provider_name in provider_names:
            ...
            if env_key in os.environ:
                ...
                logger.debug(f"Using key from environment variable for provider {provider_name}")
    ...
    def save_config(self, config: Optional[AppConfig] = None, path: Optional[str] = None):
        """保存配置到文件"""
        if config is None:
            config = self.config
    ...
    def update_llm_api_key(self, provider_name: str, api_key: str):
        ...
        if provider_name not in self.config.llm_providers:
            self.config.llm_providers[provider_name] = LLMProviderConfig(type=provider_name)
