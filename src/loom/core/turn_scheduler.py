...
class TurnScheduler:
    ...
    def __init__(self, max_concurrent: int = 3, session_manager=None, persistence_engine=None):
    ...
    async def submit_turn(self, turn: Turn) -> str:
        ...
        if turn.dependencies:
            ...
            if not all_deps_completed:
                logger.debug(f"Turn {turn.id} waiting for dependencies: {turn.dependencies}")
                return turn.id
    ...
    async def _process_turn(self, turn: Turn):
        ...
        try:
            ...
            logger.info(f"Processing turn {turn_id} (session {session_id}, turn #{turn.turn_number})")
    ...
    async def _handle_turn_timeout(self, turn: Turn):
        ...
        if turn.retry_count < turn.max_retries:
            ...
            logger.info(f"Retrying turn {turn.id} ({turn.retry_count}/{turn.max_retries})")
            ...
            for dependent_id in self.reverse_dependency[turn_id]:
                ...
                if dependent_id in self.dependency_graph:
                    ...
                    if all_deps_completed:
                        logger.debug(f"All dependencies completed for turn {dependent_id}")
    ...
    async def wait_for_turn(self, turn_id: str, timeout: Optional[float] = None) -> Optional[Turn]:
        """等待回合完成"""
        start_time = time.time()
        
        while True:
            turn = await self.get_turn(turn_id)
            if turn and turn.status in [TurnStatus.COMPLETED, TurnStatus.FAILED, TurnStatus.CANCELLED]:
                return turn
    ...
    async def cleanup_old_turns(self, max_age_hours: int = 24):
        ...
        logger.info(f"Cleaned up {len(turns_to_remove)} old completed turns from memory")
    ...
    async def get_session_turns(self, session_id: str, limit: int = 100) -> List[Turn]:
        ...
        # 从内存中获取
        for turn in list(self.completed_turns.values()) + list(self.active_turns.values()) + list(self.failed_turns.values()):
        ...
        if len(turns) < limit and self.persistence:
            turn_data_list = await self.persistence.load_turns(session_id, limit=limit - len(turns))
    ...
    def create_turn(self, session_id: str, turn_number: int, player_input: str, **kwargs) -> Turn:
        """创建新的回合对象"""
