...
from .interfaces import (
    Session, SessionConfig, SessionStatus,
    NarrativeInterpreter, NarrativeScheduler, NarrativeContext,
    NarrativeInterpretation, NarrativeArchive,
    SessionNotFoundError, NarrativeConsistencyError
)
...
class NarrativeInterpreterAdapter(NarrativeInterpreter):
    ...
    async def load_session(self, session_id: str, force_reload: bool = False) -> Optional[Session]:
        ...
        legacy_session = await self.legacy_manager.load_session(session_id, force_reload)
        if legacy_session:
    ...
    async def interpret_narrative(self, session_id: str, context: NarrativeContext) -> NarrativeInterpretation:
        ...
        return NarrativeInterpretation(
            ...
            suggested_improvements=self._suggest_improvements(consistency_score, continuity_issues),
        ...
        if "characters" in state:
            ...
            if characters:
                ...
                for char_name, char_data in list(characters.items())[:5]:
                    summary_parts.append(f"  - {char_name}: {char_data.get('role', '未知角色')}")
            ...
            for i in range(0, len(turns), 5):
                ...
                if arc_turns:
                    arcs.append({
                        ...
                        "progress": min(1.0, (i + len(arc_turns)) / max(1, len(turns))),
    ...
    def _convert_to_interface_session(self, legacy_session: Any) -> Session:
        ...
        session_dict = legacy_session.to_dict() if hasattr(legacy_session, 'to_dict') else legacy_session
    ...
    async def _analyze_consistency(self, session: Session, context: NarrativeContext) -> float:
        """分析叙事一致性"""
        # 简单实现：基于会话状态和上下文的匹配度
        state = session.state
        ...
        if "characters" in state:
            ...
            if existing_chars:
                overlap = len(existing_chars.intersection(context_chars)) / len(existing_chars)
    ...
    async def _check_continuity(self, session: Session, context: NarrativeContext) -> List[str]:
        """检查叙事连续性"""
        issues = []
        state = session.state
    ...
    async def _generate_interpretation(self, session: Session, context: NarrativeContext) -> str:
        """生成叙事解释"""
        # 基于会话状态和上下文生成解释
        state = session.state
        ...
        if arcs:
            ...
            for arc in arcs[:3]:  # 只显示前3个
                interpretation_parts.append(f"  - {arc['name']}: {arc['status']} (进度: {arc['progress']:.0%})")
        
        return "\n".join(interpretation_parts)
    
    def _suggest_improvements(self, consistency_score: float, continuity_issues: List[str]) -> List[str]:
        """基于一致性和连续性问题提出改进建议"""
        suggestions = []
...
class NarrativeSchedulerAdapter(NarrativeScheduler):
    ...
    async def schedule_narrative_event(self, session_id: str, event_type: str,
        ...
        if event_type == "time_skip":
            logger.info(f"Scheduled time skip event for session {session_id}: {event_data}")
        elif event_type == "scene_change":
            logger.info(f"Scheduled scene change event for session {session_id}: {event_data}")
        elif event_type == "character_entrance":
            logger.info(f"Scheduled character entrance event for session {session_id}: {event_data}")
        elif event_type == "character_exit":
            logger.info(f"Scheduled character exit event for session {session_id}: {event_data}")
        elif event_type == "plot_twist":
            logger.info(f"Scheduled plot twist event for session {session_id}: {event_data}")
    ...
    async def adjust_narrative_pace(self, session_id: str, pace: str) -> bool:
        ...
        if pace not in valid_paces:
            logger.warning(f"Invalid narrative pace: {pace}. Must be one of {valid_paces}")
            return False
    ...
    async def manage_narrative_dependencies(self, session_id: str) -> Dict[str, Any]:
        ...
        for event in timeline:
            ...
            if event["type"] == "scene_change":
                # 场景切换可能依赖于之前的场景
                prev_scene_events = [e for e in timeline if e["type"] == "scene_change" and e["id"] != event["id"]]
                ...
                # 情节转折可能依赖于之前的叙事弧线
                plot_events = [e for e in timeline if e["type"] in ["plot_twist", "character_development"]]
                if plot_events:
    ...
    def _check_dependency_status(self, dependency_ids: List[str], event_map: Dict[str, Dict]) -> Dict[str, str]:
        """检查依赖状态"""
        status = {}
