...
class SessionManager:
    ...
    async def create_session(self, config: SessionConfig) -> Session:
        ...
        if self.config_manager:
            ...
            if (
                not config.llm_provider
                or config.llm_provider not in app_config.llm_providers
            ):
                # 尝试根据会话类型选择Provider
                ...
                if session_type in provider_selection.session_type_mapping:
                    ...
                    if (
                        preferred_provider
                        and preferred_provider in app_config.llm_providers
                    ):
                        config.llm_provider = preferred_provider
                        config.metadata["preferred_model"] = mapping.get(
                            "preferred_model"
                        )
                        logger.info(
                            f"Selected provider {preferred_provider} for session type {session_type}"
                        )
        ...
        session = Session(
            ...
            updated_at=now,
            last_activity=now,
            state={"turns": [], "characters": {}, "locations": {}, "plotlines": []},
            metadata={**config.metadata, "created_by": "system", "version": "1.0"},
        )
    ...
    async def get_session_stats(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        turns_per_hour = session.total_turns / max(1, uptime_hours)

        # 如果结果是整数，转换为int
        if turns_per_hour.is_integer():
            turns_per_hour = int(turns_per_hour)
    ...
    async def search_sessions(self, query: Dict[str, Any]) -> List[Session]:
        ...
        for session in self.active_sessions.values():
            ...
            if "created_after" in query:
                created_after = (
                    datetime.fromisoformat(query["created_after"])
                    if isinstance(query["created_after"], str)
                    else query["created_after"]
                )
            ...
            if "created_before" in query:
                created_before = (
                    datetime.fromisoformat(query["created_before"])
                    if isinstance(query["created_before"], str)
                    else query["created_before"]
                )
