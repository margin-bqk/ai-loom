...
class WorldMemory:
    ...
    def __init__(self, session_id: str, structured_store=None, vector_store=None,
        ...
        logger.info(f"WorldMemory initialized for session {session_id}, cache={'enabled' if enable_cache else 'disabled'}")
    ...
    async def store_entity(self, entity: MemoryEntity) -> bool:
        ...
        if self.structured_store:
            ...
            if not success:
                logger.warning(f"Failed to store entity {entity.id} in structured store")
        
        # 存储到向量存储
        if self.vector_store and entity.type in [MemoryEntityType.FACT, MemoryEntityType.EVENT]:
            success = await self.vector_store.store_entity(entity)
            if not success:
                logger.debug(f"Entity {entity.id} not stored in vector store (type: {entity.type})")
        
        logger.debug(f"Stored entity {entity.id} ({entity.type.value})")
        return True
    
    async def warmup_cache(self, entity_types: Optional[List[MemoryEntityType]] = None, limit: int = 100):
        """预热缓存"""
    ...
    def _record_cache_miss(self, entity_id: str):
        ...
        self.entity_cache_misses[entity_id] = self.entity_cache_misses.get(entity_id, 0) + 1
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.cache_stats,
            ...
            "top_hits": dict(sorted(self.entity_cache_hits.items(), key=lambda x: x[1], reverse=True)[:10]),
            "top_misses": dict(sorted(self.entity_cache_misses.items(), key=lambda x: x[1], reverse=True)[:10])
        }
    ...
    async def retrieve_entities_by_type(self, entity_type: MemoryEntityType, limit: int = 100) -> List[MemoryEntity]:
        """按类型检索实体"""
        # 首先检查内存
    ...
    async def search_entities(self, query: str, filters: Optional[Dict] = None, limit: int = 10) -> List[MemoryEntity]:
        ...
        if not results and self.structured_store:
            keyword_results = await self.structured_store.search_entities(query, filters, limit)
    ...
    async def add_relation(self, relation: MemoryRelation) -> bool:
        ...
        if not source_exists or not target_exists:
            logger.warning(f"Cannot add relation: entities not found ({relation.source_id} -> {relation.target_id})")
            return False
        ...
        logger.debug(f"Added relation {relation.source_id} -[{relation.relation_type.value}]-> {relation.target_id}")
        return True
    ...
    async def update_entity(self, entity_id: str, updates: Dict[str, Any]) -> Optional[MemoryEntity]:
        """更新实体"""
        entity = await self.retrieve_entity(entity_id)
        if not entity:
            return None
    ...
    async def delete_entity(self, entity_id: str) -> bool:
        ...
        if self.structured_store:
            ...
            if not success:
                logger.warning(f"Failed to delete entity {entity_id} from structured store")
    ...
    async def import_memory(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(f"Imported memory with {len(data.get('entities', []))} entities and {len(data.get('relations', []))} relations")
            return True
    ...
    async def retrieve_entities_batch(self, entity_ids: List[str]) -> Dict[str, Optional[MemoryEntity]]:
        """批量检索实体"""
        results = {}
        ...
        logger.debug(f"Batch retrieved {len(entity_ids)} entities, {sum(1 for e in results.values() if e is not None)} found")
        return results
    
    async def update_entities_batch(self, updates: Dict[str, Dict[str, Any]]) -> Dict[str, Optional[MemoryEntity]]:
        """批量更新实体"""
        results = {}
    ...
    async def delete_entities_batch(self, entity_ids: List[str]) -> List[bool]:
        ...
        logger.info(f"Batch deleted {len(entity_ids)} entities, {sum(results)} successful")
        return results
    ...
    async def add_relations_batch(self, relations: List[MemoryRelation]) -> List[bool]:
        ...
        logger.info(f"Batch added {len(relations)} relations, {sum(results)} successful")
        return results
        ...
        try:
            for op in operations:
                ...
                if op_type == "store_entity":
                    ...
                    success = await self.add_relation(relation)
                    if not success:
                        raise Exception(f"Failed to add relation {relation.source_id}->{relation.target_id}")
                    executed_operations.append(("add_relation", f"{relation.source_id}->{relation.target_id}"))
            ...
            # 简化回滚：记录错误但不实际回滚
            # 在实际实现中，这里应该回滚已执行的操作
            logger.warning(f"Transaction rollback would be needed for {len(executed_operations)} operations")
            return False
    
    async def get_contextual_memories(self, query: str, context_entities: List[str] = None, limit: int = 10) -> List[MemoryEntity]:
        """获取上下文相关记忆"""
        results = []
        ...
        if len(results) < limit and self.structured_store:
            keyword_results = await self.structured_store.search_entities(query, None, limit * 2)
            for entity in keyword_results:
    ...
    async def get_facts(self, filters: Optional[Dict] = None, limit: int = 100) -> List[Dict[str, Any]]:
        """获取事实"""
        if not self.structured_store:
            return []
        
        try:
            facts = await self.structured_store.retrieve_facts(self.session_id, filters, limit)
            return facts
        except Exception as e:
    ...
    async def get_plotlines(self, status: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            plotlines = await self.structured_store.get_plotlines(self.session_id, status)
            return plotlines
        except Exception as e:
    ...
    async def sync_with_persistence(self, persistence_engine):
        ...
        try:
            ...
            if success:
                logger.info(f"Synced memory to persistence engine for session {self.session_id}")
            else:
    ...
    async def load_from_persistence(self, persistence_engine):
        ...
        try:
            ...
            if memory_data:
                ...
                if success:
                    logger.info(f"Loaded memory from persistence engine for session {self.session_id}")
                return success
            else:
                logger.info(f"No memory found in persistence engine for session {self.session_id}")
                return False
