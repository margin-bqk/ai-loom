...
class SummaryConfig:
    ...
    summary_strategy: str = "time_based"  # time_based, importance_based, relevance_based
...
class MemorySummarizer:
    ...
    def __init__(self, llm_provider: Optional[LLMProvider] = None, config: Optional[SummaryConfig] = None):
        self.llm_provider = llm_provider
        self.config = config or SummaryConfig()
        self.summary_cache: Dict[str, Dict[str, Any]] = {}  # cache_key -> {summary, timestamp}
        logger.info(f"MemorySummarizer initialized with strategy: {self.config.summary_strategy}")
    ...
    async def generate_summary(self, entities: List[MemoryEntity], context: Dict[str, Any] = None) -> Optional[MemorySummary]:
        ...
        if len(entities) < self.config.min_entities_to_summarize:
            logger.debug(f"Not enough entities to summarize ({len(entities)} < {self.config.min_entities_to_summarize})")
            return None
    ...
    def _generate_cache_key(self, entities: List[MemoryEntity], context: Optional[Dict[str, Any]]) -> str:
        """生成缓存键"""
        # 基于实体ID和上下文生成哈希
    ...
    async def generate_summary_with_cache(self, entities: List[MemoryEntity], context: Dict[str, Any] = None) -> Optional[MemorySummary]:
        """生成记忆摘要（带缓存）"""
        if not entities:
            return None
    ...
    def _select_entities_by_strategy(self, entities: List[MemoryEntity]) -> List[MemoryEntity]:
        """根据策略选择实体"""
    ...
    def _select_entities_time_based(self, entities: List[MemoryEntity]) -> List[MemoryEntity]:
        """基于时间选择实体（选择最旧的）"""
        sorted_entities = sorted(entities, key=lambda e: e.created_at)
        return sorted_entities[:self.config.max_entities_per_summary]
    
    def _select_entities_importance_based(self, entities: List[MemoryEntity]) -> List[MemoryEntity]:
        """基于重要性选择实体"""
        # 计算实体重要性（基于类型、元数据等）
        scored_entities = []
        ...
        for entity, score in scored_entities:
            if score >= self.config.importance_threshold and len(selected) < self.config.max_entities_per_summary:
    ...
    def _select_entities_relevance_based(self, entities: List[MemoryEntity]) -> List[MemoryEntity]:
        """基于相关性选择实体（选择相互关联的实体）"""
        # 简化实现：按时间分组
        if not entities:
            return []
        ...
        for i in range(1, len(sorted_entities)):
            time_diff = sorted_entities[i].created_at - sorted_entities[i-1].created_at
    ...
    async def generate_summary_in_format(self, entities: List[MemoryEntity], summary_format: str = None, context: Dict[str, Any] = None) -> Optional[MemorySummary]:
        """生成指定格式的摘要"""
        if not entities:
            return None
    ...
    async def _generate_structured_summary(self, entities: List[MemoryEntity], context: Optional[Dict[str, Any]]) -> Optional[MemorySummary]:
        """生成结构化摘要"""
    ...
    请生成结构化摘要："""
            ...
            logger.info(f"Generated structured summary covering {len(entities)} entities")
            return summary
    ...
    async def _generate_timeline_summary(self, entities: List[MemoryEntity], context: Optional[Dict[str, Any]]) -> Optional[MemorySummary]:
        """生成时间线摘要"""
        # 按时间排序
        sorted_entities = sorted(entities, key=lambda e: e.created_at)
        ...
        for entity in sorted_entities:
            ...
            timeline_entries.append(f"{time_str} [{entity.type.value}]: {content_preview}")
    ...
    def _prepare_summary_input(self, entities: List[MemoryEntity], context: Optional[Dict[str, Any]]) -> str:
        """准备摘要输入"""
        # 按时间排序
        sorted_entities = sorted(entities, key=lambda e: e.created_at)
...
这是一个自动生成的记忆摘要。由于未配置LLM，摘要内容较为简单。建议配置LLM提供者以获得更高质量的摘要。"""
    ...
    async def select_entities_for_summarization(self, all_entities: List[MemoryEntity]) -> List[MemoryEntity]:
        """选择需要摘要的实体"""
        if not all_entities:
            return []
        ...
        logger.debug(f"Selected {len(selected)} entities for summarization using {self.config.summary_strategy} strategy")
        return selected
    ...
    async def process_summarization(self, world_memory, force: bool = False) -> Optional[MemorySummary]:
        ...
        if force:
            ...
            entities_to_summarize = sorted_entities[:self.config.max_entities_per_summary]
        else:
            entities_to_summarize = await self.select_entities_for_summarization(all_entities)
        ...
        if summary:
            ...
            logger.info(f"Summarization completed: {len(entities_to_summarize)} entities summarized")
