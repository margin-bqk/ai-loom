...
class StructuredStore:
    ...
    def __init__(
        ...
        self._ensure_tables()
        logger.info(
            f"StructuredStore initialized with db={db_path}, cache={'enabled' if enable_cache else 'disabled'}"
        )
    ...
    def _ensure_tables(self):
        ...
        def create_tables():
            ...
            # 创建索引
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_entities_session ON memory_entities (session_id)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_entities_type ON memory_entities (type)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_entities_active ON memory_entities (is_active)"
            )
            cursor.execute(
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_facts_timestamp ON memory_facts (timestamp)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_facts_type ON memory_facts (fact_type)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_plotlines_session ON plotlines (session_id)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_plotlines_status ON plotlines (status)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_entity_versions_entity ON entity_versions (entity_id)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_associations_entity ON entity_fact_associations (entity_id)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_associations_fact ON entity_fact_associations (fact_id)"
            )
    ...
    async def retrieve_entities_by_type(
        self, session_id: str, entity_type: MemoryEntityType, limit: int = 100
    ) -> List[MemoryEntity]:
        """按类型检索实体"""
    ...
    async def retrieve_facts(
        self, session_id: str, filters: Optional[Dict] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """检索事实"""
            ...
            for entity_assoc in plotline_data.get("entities", []):
                ...
                """,
                    (
                        ...
                        json.dumps(
                            entity_assoc.get("metadata", {}), ensure_ascii=False
                        ),
                    ),
                )
    ...
    async def update_plotline(self, plotline_id: str, updates: Dict[str, Any]) -> bool:
        ...
        def update():
            ...
            if "entities" in updates:
                # 先删除现有关联
                cursor.execute(
                    "DELETE FROM plotline_entities WHERE plotline_id = ?",
                    (plotline_id,),
                )
    ...
    async def get_related_facts(
        self, entity_id: str, association_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """获取相关事实"""
    ...
    async def retrieve_entities_by_type(
        ...
        if self.enable_cache:
            cache_key = self._get_cache_key(
                "retrieve_entities_by_type", session_id, entity_type.value, limit
            )
            cached_result = self._get_cached_result(cache_key)
            if cached_result is not None:
                return cached_result
    ...
    def _invalidate_entity_cache(
        self, entity_id: str, session_id: str, entity_type: MemoryEntityType
    ):
        """使实体相关缓存失效"""
        # 清除实体检索缓存
        ...
        # 清除按类型检索缓存
        type_cache_key = self._get_cache_key(
            "retrieve_entities_by_type", session_id, entity_type.value, 100
        )
