...
class VectorStore:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.embedding_provider = self.config.get(
    ...
    def _initialize(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Loaded existing vector collection: {self.collection_name}"
                )
            except:
            ...
            logger.info(
                f"VectorStore initialized successfully with provider: {self.embedding_provider}"
            )
    ...
    def _get_openai_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            response = openai.Embedding.create(model=self.openai_model, input=text)
    ...
    async def search(self, query: str, limit: int = 10) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            logger.debug(
                f"Vector search found {len(search_results)} results for query: {query[:50]}..."
            )
    ...
    async def search_by_entity(
        self, entity: MemoryEntity, limit: int = 5
    ...
    async def sync_with_structured_store(
        self,
        structured_store,
        session_id: str,
        ...
        try:
            ...
            for entity_type in types_to_sync:
                entities = await structured_store.retrieve_entities_by_type(
                    session_id, entity_type, limit=1000
                )
            ...
            logger.info(
                f"Synced {synced_count} entities from structured store to vector store"
            )
            return True
