...
async def read_root(request: Request):
    ...
    return templates.TemplateResponse(
        "index.html",
        {"request": request, "title": "LOOM Web UI"}
    )
...
async def list_sessions(include_inactive: bool = False):
    ...
    if not session_manager:
        return JSONResponse(
            status_code=503,
            content={"error": "会话管理器未初始化"}
        )
    
    try:
        sessions = await session_manager.list_sessions(include_inactive=include_inactive)
        sessions_list = []
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"获取会话列表失败: {str(e)}"}
        )
...
async def list_rules():
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"获取规则列表失败: {str(e)}"}
        )
...
async def get_rule(canon_name: str):
    ...
    try:
        ...
        if not canon:
            return JSONResponse(
                status_code=404,
                content={"error": f"规则集 {canon_name} 不存在"}
            )
