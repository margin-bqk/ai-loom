...
class DeploymentVerifier:
    ...
    def verify_local_installation(self) -> bool:
        ...
        # 检查 Python 包
        success, output = self.run_command("python -c \"import loom; print(loom.__version__)\"")
        ...
        # 检查 Web 服务器
        success, output = self.run_command("python -c \"from loom.web.app import app; print('Web app loaded')\"")
    ...
    def verify_docker_deployment(self) -> bool:
        ...
        # 运行容器并检查健康状态
        success, output = self.run_command("docker run -d -p 8000:8000 --name loom-test loom-test")
        if success:
            time.sleep(5)  # 等待容器启动
    ...
    def verify_kubernetes_deployment(self) -> bool:
        ...
        if config_path.exists():
            ...
            # 检查配置文件语法
            files = ["namespace.yaml", "configmap.yaml", "deployment.yaml", "service.yaml"]
            for file in files:
                file_path = config_path / file
                if file_path.exists():
                    success, output = self.run_command(f"kubectl apply --dry-run=client -f {file_path}")
    ...
    def generate_report(self, output_file: Optional[Path] = None) -> Dict:
        ...
        report = {
            ...
            "summary": "所有测试通过" if all(r["success"] for r in self.results) else "部分测试失败"
        }
...
def main():
    ...
    if not any([args.local, args.docker, args.kubernetes, args.monitoring, args.cloud_endpoints, args.all]):
        parser.print_help()
        sys.exit(1)
