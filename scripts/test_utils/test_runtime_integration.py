...
async def test_component_interactions():
    ...
    try:
        ...
        persistence = SQLitePersistence(db_path=db_path)
        await persistence.initialize()
        session_manager = SessionManager(persistence_engine=persistence, config_manager=config_manager)
        turn_scheduler = TurnScheduler(max_concurrent=2, session_manager=session_manager, persistence_engine=persistence)
        prompt_assembler = PromptAssembler(config_manager=config_manager)
        ...
        session = await session_manager.create_session(session_config)
        default_provider = config_manager.get_config().session_defaults.default_llm_provider
        if session.config.llm_provider == default_provider:
            print(f"   ✓ 会话使用了配置管理器中的默认LLM提供商: {default_provider}")
        else:
            print(f"   ✗ 会话LLM提供商不匹配: {session.config.llm_provider} != {default_provider}")
