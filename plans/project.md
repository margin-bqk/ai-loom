...
**LOOM 是一个叙事解释器运行时：世界由文本定义，叙事由解释生成，运行时只负责维持叙事的连续性。**

---

## 1. 项目定位（Project Positioning）

LOOM 是一个 **LLM 原生（LLM-native）** 的叙事解释器运行时，用于：
...
1. **规则=文本**：所有规则都以纯文本形式存在，永不编译为代码
2. **世界=持续对话**：世界状态通过对话演进，而非状态机转换
3. **引擎只负责不让一切崩塌**：运行时确保叙事连续性，不参与叙事创作
