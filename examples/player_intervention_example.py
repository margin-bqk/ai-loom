...
async def example_player_intervention():
    ...
    for result in intervention_results:
        ...
        print(
            f"  {result.intervention.type.value}: {status} - {result.narrative_impact}"
        )
    ...
    for i, interv in enumerate(prioritized, 1):
        print(
            f"  {i}. {interv.type.value} (优先级: {interv.priority.name}, 紧急度: {interv.urgency})"
        )
    ...
    if conflict_parsed["conflicts"]:
        ...
        for conflict in conflict_parsed["conflicts"]:
            print(
                f"  {conflict['intervention1']} 与 {conflict['intervention2']}: {conflict['reason']}"
            )
    else:
        print("无冲突")
