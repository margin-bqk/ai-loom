...
async def run_fantasy_example():
    ...
    response1 = await session_manager.process_turn(
        session_id,
        "艾莉亚醒来，发现自己躺在法师塔的床上。窗外传来鸟鸣声。她应该做什么？"
    )
    ...
    response3 = await session_manager.process_turn(
        session_id,
        "在塔顶，雷纳德告诉艾莉亚今天有一个重要的魔法测试。她感到既紧张又兴奋。"
    )
    print(f"AI 响应: {response3[:100]}...")
    
    # 6. 保存会话
    print("\n6. 保存会话...")
    await session_manager.save_session(session_id, "examples/full_example/sessions/fantasy_session.json")
...
async def demonstrate_features():
    ...
    if sessions:
        ...
        for i, session in enumerate(sessions[:3], 1):
            print(f"  {i}. {session.get('id', '未知')} - {session.get('world_name', '未知世界')}")
