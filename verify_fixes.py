        ...
        try:
            ...
            if result.returncode == 0:
                self.report.add_result(VerificationResult(
                    ...
                    status=VerificationStatus.PASS,
                    message="CLI命令可正常执行",
                    details={"command": "loom --version", "output": result.stdout.strip()},
                    timestamp=datetime.now().isoformat()
                ))
            else:
                self.report.add_result(VerificationResult(
...
class QuickStartVerifier(BaseVerifier):
    ...
    def _verify_quickstart_docs(self):
        ...
        if quickstart_docs.exists():
            ...
            required_docs = ["install-and-run.md", "basic-configuration.md", "first-example.md"]
            ...
            try:
                ...
                if len(found_commands) >= 2:  # 至少找到2个命令
                    ...
                    self.report.add_result(VerificationResult(
                        ...
                        details={
                            "found_commands": found_commands,
                            "missing_commands": list(set(required_commands) - set(found_commands))
                        },
                        timestamp=datetime.now().isoformat()
                    ))
    ...
    def _verify_example_code(self):
        ...
        if examples_dir.exists():
            example_files = list(examples_dir.glob("*.py")) + list(examples_dir.glob("*.md"))
                    ...
                    if example_file.suffix == '.py':
                        try:
                            ...
                            self.report.add_result(VerificationResult(
                                ...
                                details={"file": example_file.name, "error": str(e)},
                                timestamp=datetime.now().isoformat()
                            ))
                        except Exception as e:
                            self.report.add_result(VerificationResult(
...
def main():
    ...
    parser.add_argument("--category", choices=["all", "docs", "docker", "cli", "quickstart", "scripts"],
                       default="all", help="验证类别")
    parser.add_argument("--output", choices=["console", "json", "both"],
                       default="console", help="输出格式")
    parser.add_argument("--output-file", default="verification_report.json",
                       help="JSON输出文件路径")
